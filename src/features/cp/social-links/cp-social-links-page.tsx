...
import { Plus, GripVertical, Trash2, Pencil, ExternalLink, X, Link2, Loader2 } from 'lucide-react'
import { PageHeader } from '@/components/shared/page-header'
import { ConfirmDialog } from '@/components/shared/confirm-dialog'
import { useOrderedList } from '../lib/use-ordered-list'
import { socialLinksHooks, type CpSocialLink } from '../lib/cp-hooks'

const PLATFORMS = ['instagram', 'linkedin', 'facebook', 'twitter', 'youtube', 'tiktok'] as const
...
export function CpSocialLinksPage() {
  const { data: serverLinks, isLoading } = socialLinksHooks.useList()
  const createLink = socialLinksHooks.useCreate()
  const updateLink = socialLinksHooks.useUpdate()
  const deleteLink = socialLinksHooks.useRemove()
  const reorderLinks = socialLinksHooks.useReorder()
  ...
  function openEdit(link: CpSocialLink) {
    setTitle(link.title || '')
    setUrl(link.url || '')
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (editingId) await updateLink.mutateAsync({ id: editingId, title, url, platform })
    else await createLink.mutateAsync({ title, url, platform })
    setShowForm(false)
  }
    ...
    <div>
      ...
      {showForm && (
        <div className="mb-6 bg-card rounded-2xl border border-border/60 p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold">{editingId ? 'Edit Social Link' : 'New Social Link'}</h3>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} required className={inputClass} placeholder="Instagram" />
              </div>
              <div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <p className="text-muted-foreground italic">No social links yet. Add your first link.</p>
        </div>
      ) : (
        <DragDropContext onDragEnd={onDragEnd}>
          <Droppable droppableId="cp-social-links">
            {(provided) => (
                    ...
                    {(provided, snapshot) => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="font-medium text-sm truncate">{link.title || link.platform}</div>
